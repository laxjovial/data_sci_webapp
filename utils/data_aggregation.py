...
def group_by_aggregate(df, group_by_cols, agg_dict):
    """
    Performs a groupby operation with multiple aggregations on a Pandas DataFrame.

    Technical: This function leverages Pandas' groupby() method and the .agg() method,
    ...
    Returns:
        pd.DataFrame: The aggregated Pandas DataFrame.
        str: An error message if the operation fails, otherwise None.
    """
    if not isinstance(df, pd.DataFrame):
        return None, "Error: Invalid input Pandas DataFrame."
...
def create_pivot_table(df, index, columns, values, aggfunc):
    """
    Creates a pivot table from a Pandas DataFrame.

    Technical: This is a wrapper around the Pandas `pivot_table` method. It allows
